...
def edit_planning_dialog(planning_obj):
    with ui.dialog() as dialog, ui.card():
        ...
        all_depots = get_depots() # Mostrar todos para contexto, incluindo o atual se inativo
...
def planning_list_ui():
    ...
    with _planning_list_container, ui.card().classes("w-full h-full overflow-auto"):
        ...
        plannings = get_planning() # Mostrar todos para gerenciamento
        if plannings:
            ui.separator()
            for p in plannings:
